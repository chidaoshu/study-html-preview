  ...
  document.querySelectorAll('.lab').forEach(function (lab) {
    ...
    body.innerHTML =
      '<div class="ed-wrap">' +
        ...
        '<div class="ed-panes">' +
          '<div class="ed">' +
            '<button class="ibtn" data-act="copy" aria-label="コードをコピー"><svg viewBox="0 0 24 24"><rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg></button>' +
          ...
          '</div>' +
          '<div class="demo-col"><div class="frame-wrap"><iframe class="lab-frame" aria-label="デモの表示"></iframe></div></div>' +
        '</div>' +
      '</div>';
    ...
    function foldLines(cls, start, hidden, key) {
      ...
      if (!expanded) {
        ...
        head.appendChild(dots);
         head.setAttribute('aria-label', hidden.length + ' 行を展開');
      } else {
         head.setAttribute('aria-label', '折りたたむ');
      }
    }
